...
def get_layer_sparsity(layer_max_sparsity, t, tmax, sparsity_level_function='smoothstep'):
    """
    Returns the value of the `sparsity_level_function` function at
    `t / tmax` multiplied by `layer_max_sparsity`.
    
    If 'smoothstep' is chosen, then as `t` approaches `tmax`, the
    result of this function smoothly approaches `layer_max_sparsity`.

    Args:
        layer_max_sparsity: The final (max) fraction of neurons that should
            be sparsified.
        t: The current timestep.
        tmax: The final timestep.
        sparsity_level_function: The function that maps `t/tmax` to the desired
            sparsity level.

    Returns:
        The desired sparsity fraction at time `t`.
    """

    if sparsity_level_function == 'smoothstep':
        return mutils.smoothstep(t / tmax) * layer_max_sparsity
    elif sparsity_level_function == 'linear':
        return (t / tmax) * layer_max_sparsity
    elif sparsity_level_function == 'constant':
        return layer_max_sparsity
    else:
        raise ValueError(f"{sparsity_level_function=} not supported")

def sparsifier(layer_v_to_sparsify, layer_v_deciding_sparsity, layer_sparsity, sparsity_type='step',
               maintain_total_activity=False, parsity_alpha=1, eps = 1e-8):
    """
    Takes a tensor of layer activations `layer_v` and a desired sparsity fraction
    `layer_sparsity` and returns a sparsified version of the pre-nonlinearity
    layer activations according to the given `sparsity_type`.

    Args:
        layer_v_to_sparsify of shape `(batch_size, num_hidden_neurons)`:
            The somatic voltage activations of the current layer to be
            sparsified.
        layer_v_deciding_sparsity of shape `(batch_size, num_hidden_neurons)`:
            The neuron activations of the current layer that determine which
            neurons are sparsified. Can be the same as `layer_v_to_sparsify`,
            but doesn't have to be.
        layer_sparsity: The fraction of neurons in the layer that should
            be sparsified.
        sparsity_type: The type of sparsity to be used ('step' or 'sigmoid')
        sparsity_alpha: The parameter defining the shape of the sigmoid.
        eps: A small number to avoid sparsifying lowest activity even
            if `layer_sparsity == 0`.
    """

    abs_values = torch.abs(layer_v_deciding_sparsity)

    quantiles = torch.quantile(abs_values, layer_sparsity, dim=1)
    quantiles = quantiles.reshape(len(quantiles), 1)
    if sparsity_type in ['step', 'clamp_step']:
        sparsity_multiplier = torch.heaviside(
            abs_values - quantiles + eps,
            torch.tensor([0], dtype=layer_v_deciding_sparsity.dtype,
                         device=layer_v_deciding_sparsity.device)
        )
    elif sparsity_type == 'sigmoid':
        sparsity_multiplier = 1 / (1 + torch.exp(-sparsity_alpha * (abs_values - quantiles)))
    else:
        raise NotImplementedError()
    
    if sparsity_type == 'clamp_step':
        sparse_v = sparsity_multiplier
    else:
        sparse_v = layer_v_to_sparsify * sparsity_multiplier
    

    if maintain_total_activity:
        sparse_v = sparse_v * torch.norm(layer_v_to_sparsify) / torch.norm(sparse_v)
