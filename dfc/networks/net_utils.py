...
clean.
"""
from networks import bp_network, dfc_network,\
                     dfc_network_single_phase, ewc_network_wrapper,\
                     bp_network_ext_wrapper, si_network_wrapper,\
                     l2_network_wrapper
...
def generate_network(config, dataset, device, network_type='BP',
    ...
    print(config.lr)
    kwconfig_ext = {'lr': config.lr[0], 'momentum': config.momentum,
                    'weight_decay': 0,'schedule': [config.epochs],
                    'model_type':'mlp', 'model_name': f'MLP{config.size_hidden[0]}',
                    'model_weights':None,
                    'out_dim':{'All':dataset.out_size},
    ...
    if network_type == 'BP':
    ...
    elif network_type == 'EWC':
        net = ewc_network_wrapper.EWCNetwork(agent_config=kwconfig_ext)

    elif network_type == 'SI':
        net = si_network_wrapper.SINetwork(agent_config=kwconfig_ext)

    elif network_type == 'L2':
        net = l2_network_wrapper.L2Network(agent_config=kwconfig_ext)
    
    elif network_type == 'BPExt':
        net = bp_network_ext_wrapper.BPNetworkExt(agent_config=kwconfig_ext)
        ...
        kwconfig_dfc_base = {
                ...
                'layer_max_sparsities': config.layer_max_sparsities,
                'maintain_total_activity': config.maintain_total_activity,
                'freeze_suppressed_neuron_weights': config.freeze_suppressed_neuron_weights,
                'strong_feedback': config.strong_feedback,
                'use_recurrent_weights': config.use_recurrent_weights,
                'rec_learning_neurons': config.rec_learning_neurons,
                'rec_grad_normalization': config.rec_grad_normalization,
                'fw_grad_normalization': config.fw_grad_normalization,
                'sparsity_level_function': config.sparsity_level_function,
                'rec_inference_iterations': config.rec_inference_iterations,
                'frac_fb_deciding_sparsity': config.frac_fb_deciding_sparsity,
                'frac_rec_deciding_sparsity': config.frac_rec_deciding_sparsity,
                'sparsity_type': config.sparsity_type,
                'turn_off_rec_norm_normalization': config.turn_off_rec_norm_normalization,
                'presynaptic_inhibition': config.presynaptic_inhibition,
                'hebbian_fw_learning': config.hebbian_fw_learning,
                'from_ff_learning': config.from_ff_learning}
    ...
    if (logger is not None) and (not network_type in ['EWC', 'SI', 'BPExt', 'L2']):
