    ...
    * ``controller``: a proportional integral controller.
    ...
    Args:
        ...
        learning_rule (str): The type of learning rule to use for the forward
            weights.
        layer_max_sparsities (list): Layer-wise maximum fractions of neurons whose
            voltage will be sparsified (set to 0) during training and inference.
            If not provided, no sparsity will be used.
        maintain_total_activity (bool): Whether to scale sparsified activations such
            as to maintain the same norm as the pre-sparsified activations.
            Generally not used.
        freeze_suppressed_neuron_weights (bool):
            Whether to block learning of weights from neurons that were suppressed by
            the sparsity mechanism.
        use_recurrent_weights (bool): Whether to use recurrent weights for gating.
        rec_learning_neurons (str): Indicates which neurons have their incoming
            recurrent weights updated.
        rec_grad_normalization (str): Indicates the type of normalization applied
            to recurrent weight updates.
        fw_grad_normalization (str): Indicates the type of normalization applied
            to forward weight updates.
        sparsity_level_function (str): The name of the function that maps elapsed
            time to the desired sparsity level within one training iteration.
        sparsity_type (str): The function deciding how activities should be sparsified.
        rec_inference_iterations (int): The number of iterations we simulate to
            compute the effect of recurrent gating during inference. (generally set to 0)
        frac_fb_deciding_sparsity (float): The fraction that feedback should take up in
            the activity that decides which neurons to sparsify. Not used in normal
            training.
        frac_rec_deciding_sparsity (float): The fraction of pure feedforward activity which
            accounts for recurrent activity that decides which neurons to sparsify.
            Not used in normal training.
        turn_off_rec_norm_normalization (bool): Whether to Turn off rescaling neuron activations
            after applying recurrent inhibition. Generally not used.
        presynaptic_inhibition (bool): Whether to only consider presynaptic activity instead of
            also taking into account postsynaptic v_ff, when computing recurrent weight updates.
        hebbian_fw_learning (bool): Whether to update incoming weights to non-sparsified neurons
            in a hebbian fashion rather than comparing their somatic to their feedforward activity.
            Generally not used.
        from_ff_learning (bool): Whether weight updates are performed with respect to pure
            feedforward activations as opposed to activations incorporating recurrent activity.
            Generally not used.
    """
    def __init__(self, n_in, n_hidden, n_out, activation='relu', bias=True,
                 ...
                 sparsity_level_function='smoothstep',
                 sparsity_type='step',
                 rec_inference_iterations=0,
                 frac_fb_deciding_sparsity=-1,
                 frac_rec_deciding_sparsity=-1,
        ...
        super().__init__(n_in, n_hidden, n_out, activation=activation,
                         ...
                         use_recurrent_weights=use_recurrent_weights)
        ...
        self.layer_max_sparsities = layer_max_sparsities
        self.maintain_total_activity = maintain_total_activity
        self.freeze_suppressed_neuron_weights = freeze_suppressed_neuron_weights
        self.use_recurrent_weights = use_recurrent_weights
        self.rec_learning_neurons = rec_learning_neurons
        self.rec_grad_normalization = rec_grad_normalization
        self.fw_grad_normalization = fw_grad_normalization
        self.sparsity_level_function = sparsity_level_function
        self.rec_inference_iterations = rec_inference_iterations
        self.frac_fb_deciding_sparsity = frac_fb_deciding_sparsity
        self.frac_rec_deciding_sparsity = frac_rec_deciding_sparsity
        self.sparsity_type = sparsity_type
        self.turn_off_rec_norm_normalization = turn_off_rec_norm_normalization
        self.presynaptic_inhibition = presynaptic_inhibition
        self.hebbian_fw_learning = hebbian_fw_learning
        self.from_ff_learning = from_ff_learning
        if compare_with_ndi:
            assert not ndi
    ...
    def state_dict(self):
        ...
        including forward, backward and recurrent weights.

        Returns:
            (dict): The forward, feedback and recurrent weights.
        """
        forward_weights = [layer.weights.data for layer in self.layers]
        feedback_weights = [layer.weights_backward.data for layer in \
            self.layers]
        recurrent_weights = [layer.weights_recurrent.data for layer in \
            self.layers]
    ...
    def load_state_dict(self, state_dict):
        ...
        Args:
            state_dict (dict): The state with forward, backward and recurrent weights.
        """
        for l, layer in enumerate(self._layers):
            ...
            layer.weights_recurrent.data = state_dict['recurrent_weights'][l]
    ...
    def forward(self, x, sparsity=False):
        """Compute the output :math:`y` of this network given the input
        :math:`x`.

        Args:
            x (torch.Tensor): The input to the network.
            sparsity (bool): Whether sparsity should be
                applied in this forward pass.
        ...
        for i, layer in enumerate(self.layers):
            ...
            if self.layer_max_sparsities and sparsity:
                lms = self.layer_max_sparsities[i]
                layer_sparsity = get_layer_sparsity(lms, 1, 1, self.sparsity_level_function)
            else: 
                layer_sparsity = 0

            y = layer.forward(y,
                              rec_inference_iterations=self.rec_inference_iterations,
                              layer_sparsity=layer_sparsity,
                              maintain_total_activity=self.maintain_total_activity,
                              dt_di=self.dt_di, time_constant_ratio=self.time_constant_ratio,
                              turn_off_rec_norm_normalization=self.turn_off_rec_norm_normalization)
    ...
    def backward(self, loss, targets=None, return_for_fb=False, verbose=True):
        ...
        for i in range(self.depth):
            ...
            if self.cont_updates:
                ...
                if self.layer_max_sparsities:
                    # create boolean array of sparsified neurons for subsequent weight freezing of forward weights
                    suppressed_neurons = v[i] == 0
                else:
                    suppressed_neurons = None
                self.layers[i].compute_forward_gradients(delta_v_i, r_previous, r[i],
                            suppressed_neurons=suppressed_neurons,
                            freeze_suppressed_neuron_weights=self.freeze_suppressed_neuron_weights,
                            fw_grad_normalization=self.fw_grad_normalization,
                            learning_rule=self.learning_rule,
                            turn_off_rec_norm_normalization=self.turn_off_rec_norm_normalization,
                            hebbian_fw_learning=self.hebbian_fw_learning,
                            from_ff_learning=self.from_ff_learning)
                
                if self.use_recurrent_weights:
                    self.layers[i].compute_recurrent_gradients(delta_v_i, r_previous, r[i],
                                suppressed_neurons=suppressed_neurons,
                                learning_rule=self.learning_rule,
                                rec_learning_neurons=self.rec_learning_neurons,
                                rec_grad_normalization=self.rec_grad_normalization,
                                turn_off_rec_norm_normalization=self.turn_off_rec_norm_normalization,
                                presynaptic_inhibition=self.presynaptic_inhibition)
        ...
        for tensor_list in [u_time, v_fb_time, v_time, v_ff_time, r_time]:
    ...
    def non_dynamical_inversion(self, output_target, retain_graph=False):
        ...
        feedback). For the other symbols, refer to https://arxiv.org/pdf/2204.07249.pdf.
    ...
    def controller(self, output_target, alpha, dt, tmax, k_p=0.,
        ...
        for t in range(tmax - 1):
            ...
            def layer_iteration(i):
                ...
                a_ff_pre_rec = a.clone()
                if self.use_recurrent_weights:
                    a = self.layers[i].recurrent_activation(a, r[i][t], turn_off_rec_norm_normalization=self.turn_off_rec_norm_normalization)
                ...
                # v[i][t + 1, :] has shape (batch_size, num_neurons_in_layer)
                if self.inst_system_dynamics:
                ...
                # sparsify somatic voltage activations if necessary
                if self.layer_max_sparsities:
                    # get maximal (final) sparsity for layer i
                    lms = self.layer_max_sparsities[i]

                    # get the desired sparsity for layer i at timestep t
                    layer_sparsity = get_layer_sparsity(lms, t, tmax, self.sparsity_level_function)

                    # if args were provided, mix feedforward, recurrent and feedback activity with 
                    # specific fractions to determine sparsity
                    if self.frac_fb_deciding_sparsity >= 0 and self.frac_rec_deciding_sparsity >= 0:
                        v_ff_mag = (v_ff[i][t + 1, :] * self.frac_rec_deciding_sparsity + \
                                    a_ff_pre_rec * (1 - self.frac_rec_deciding_sparsity)).abs()
                        v_fb_mag = v[i][t + 1, :].abs() - v_ff[i][t + 1, :].abs()
                        layer_v_deciding_sparsity = torch.clip(v_fb_mag * self.frac_fb_deciding_sparsity + \
                                                               v_ff_mag * (1 - self.frac_fb_deciding_sparsity),
                                                               min=0)
                    else:
                        layer_v_deciding_sparsity = v[i][t + 1, :]

                    # sparsify pre-nonlinearity activations
                    v[i][t + 1, :] = sparsifier(v[i][t + 1, :], layer_v_deciding_sparsity, layer_sparsity,
                                                maintain_total_activity=self.maintain_total_activity,
                                                sparsity_type=self.sparsity_type)

                # Compute low-pass filter if needed for jacobian.
                if noisy_dynamics and self.use_jacobian_as_fb:
    ...
    def compute_loss(self, output_target, r, axis=1):
        ...
        By default this error is computed as in https://arxiv.org/pdf/2204.07249.pdf
    ...
    def recurrent_params(self):
        """Access recurrent parameters.

        Returns:
            params (list): The list of recurrent parameters.
        """
        params = []
        for layer in self.layers:
            params.append([layer.weights_recurrent])
    ...
    def params(self):
        ...
        if self.use_recurrent_weights:
            params.extend(self.recurrent_params)

        return params

    def get_max_grad(self, params_type='all'):  
        """Return the maximum gradient across the parameters.

        Args:
            params_type (str): Whether to compute across the entire set of
                parameters or only forward or only feedback or only recurrent.
        ...
        elif params_type == 'recurrent':
            return super().get_max_grad(params=self.recurrent_params)
    ...
    def to(self, device):
        """Override `to` method to also move the backward and recurrent weights."""
        super().to(device)
        for i in range(self.depth):
            self.layers[i].weights_backward = \
                self.layers[i].weights_backward.to(device)
            if self.use_recurrent_weights:
                self.layers[i].weights_recurrent = \
                    self.layers[i].weights_recurrent.to(device)
