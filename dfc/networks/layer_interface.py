...
class LayerInterface(nn.Module, ABC):
    ...
    def __init__(self, in_features, out_features, last_layer_features=None,
        ...
        self._weights.data = self._weights - self._weights.mean(dim=1).reshape(-1, 1)
        #self._weights.data = self._weights / self._weights.norm(dim=1).reshape(-1, 1)
    ...
    def init_layer(self, weights, bias=None, initialization='xavier'):
        ...
        if initialization == 'orthogonal':
            ...
            torch.nn.init.constant_(weights, 0)
        else:
