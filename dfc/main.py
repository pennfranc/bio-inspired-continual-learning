...
def run(network_type='BP'):
    ...
    if config.dataset in ['split_mnist', 'permuted_mnist', 'split_cifar', 'split_fashion_mnist', 'split_kmnist']:
        ...
        shared.continual_learning = False

    ### Create the task.
    dloader = generate_task(config, logger, device)
    if shared.continual_learning:
        dloaders = dloader
        dloader = dloaders[0]
    ...
    if not (network_type in ['EWC', 'SI', 'BPExt', 'L2']):
    ...
    ### Train the network.
    if shared.continual_learning:
        shared = tutils.train_incremental(config, logger, device,
                                          writer, dloaders, net,
                                          optimizers, shared, network_type,
                                          loss_fn)
    else: 
    ...
    #torch.save(net.state_dict, config.out_dir + '/state_dict.pt')
