...
def parse_cmd_arguments(network_type='BP', default=False, argv=None):
    ...
    if network_type == 'DFC':
        optimizer_args(tgroup, suffix='_fb', dweight_decay=0.01, dmomentum=None,
                       doptimizer=None)
        optimizer_args(tgroup, suffix='_rec', dweight_decay=0)
        dfc_args(parser)
...
def training_args(parser, network_type, depochs=2, dbatch_size=128,
    ...
    tgroup.add_argument('--epochs', type=int, metavar='N', default=depochs,
                        help='Number of training epochs. ' +
                             'Default: %(default)s.')
    tgroup.add_argument('--limited_batch_nr', type=int, default=-1,
                        help='If positive, maximum number of batches to train '
                             'on per epoch.')
    ...
    tgroup.add_argument('--clip_grad_norm', type=float, default=dclip_grad_norm,
                        help='Clip the norm of the forward and feedback weight '
                             ...
                             'value.')
    tgroup.add_argument('--cl_mode', type=str, default='domain',
                        choices=['domain', 'class'],
                        help="The mode of incremental learning that will be "
                              "used. Should be equal to 'domain' or 'class'.")
    tgroup.add_argument('--num_tasks', type=int, default=5,
                        help='Number of different tasks for continual learning.'
    ...
    tgroup.add_argument('--min_acc', type=float, default=0.,
                        help='The minimal accuracy that needs to be achieved '
                             ...
                             'killed.')
    tgroup.add_argument('--stop_early_at_accu', type=float, default=2,
...
def optimizer_args(tgroup, suffix='', dlr='0.1', dmomentum=0, dweight_decay=0,
    ...
    if suffix == '_fb':
        ...
        name = 'recurrent'
        explanation = ' for the recurrent weights'
...
def miscellaneous_args(mgroup):
    ...
    mgroup.add_argument('--record_first_batch_activations', action='store_true',
                        help='If activated, activations/targets of first test batches '
                             'will be saved.')

def network_args(parser, dsize_hidden='5', dhidden_activation='relu',
    ...
    sgroup.add_argument('--reg_coef', type=float, default=-1,
                        help='The coefficient of the loss regularization term. '
                             'Only used for EWC and SI.')
...
def dfc_args(parser, dinit_fb_epochs=1,  dsigma_init=0, dextra_fb_epochs=0,
    ...
    tgroup.add_argument('--fw_grad_normalization', type=str, default='',
                        choices=['incoming', ''],
                        help="Optionally, specifies how forward weight updates are normalized.")
    ...
    # Sparsity.
    tgroup.add_argument('--layer_max_sparsities', type=str, default=None,
                        help='Layer-wise maximum fractions of neurons whose voltage '
                             'will be sparsified (set to 0) during training and inference. '
                             'If not provided, no sparsity will be used.')
    tgroup.add_argument('--maintain_total_activity', action='store_true',
                        help='Whether to scale sparsified activations such as to '
                             'maintain the same norm as the pre-sparsified activations.')
    tgroup.add_argument('--freeze_suppressed_neuron_weights', action='store_true',
                        help='Whether to block learning of weights from neurons that '
                             'were suppressed by the sparsity mechanism.')
    tgroup.add_argument('--block_non_sparsified_neurons', action='store_true',
                        help='UNUSED')
    tgroup.add_argument('--sparsity_level_function', type=str, default='smoothstep',
                        help='The name of the function that maps elapsed time to the desired '
                             'sparsity level within one training iteration.')
    tgroup.add_argument('--sparsity_type', type=str, default='step',
                        help='The function deciding how activities should be sparsified.',
                        choices=['step', 'clamp_step', 'sigmoid'])
    tgroup.add_argument('--permanent_sparsification', action='store_true',
                        help='Activating this option leads to reducing the number of neurons '
                             'by the sparsity fraction on each layer, thus leading to a '
                             'permanent sparsification of the network.')    
    tgroup.add_argument('--frac_fb_deciding_sparsity', type=float, default=-1,
                        help="The fraction that feedback should take up in the activity "
                              "that decides which neurons to sparsify.")   
    tgroup.add_argument('--frac_rec_deciding_sparsity', type=float, default=-1,
                        help="The fraction of feedforward activity which accounts for recurrent "
                              "activity that decides which neurons to sparsify.") 
    tgroup.add_argument('--hebbian_fw_learning', action='store_true',
                        help='Whether to update incoming weights to non-sparsified neurons in a '
    ...
    tgroup.add_argument('--use_recurrent_weights', action='store_true',
                        help='Add recurrent weights to every layer.')   
    tgroup.add_argument('--rec_learning_neurons', type=str, default='all',
                        choices=['all', 'active_only', 'suppressed_only', ''],
                        help="Which neuron's incoming weights should be updated.") 
    tgroup.add_argument('--rec_grad_normalization', type=str, default='',
                        choices=['incoming', 'outgoing', 'all', ''],
                        help="Optionally, specifies how rec weight updates are normalized.")
    tgroup.add_argument('--rec_adaptation', type=float, default=-1,
                        help="UNUSED") 
    tgroup.add_argument('--rec_inference_iterations', type=int, default=0,
                        help="The number of iterations to play out the recurrent dynamics "
                             "at inference time.") 
    tgroup.add_argument('--turn_off_rec_norm_normalization', action='store_true',
                        help='Turn off rescaling neuron activations after applying recurrent inhibition.') 
    tgroup.add_argument('--presynaptic_inhibition', action='store_true',
                        help='Whether to only consider presynaptic activity, '
...
def post_process_args(config, network_type):
    ...
    if network_type == 'DFC' or network_type == 'DFC_single_phase':
        ...
        if config.layer_max_sparsities:
            config.layer_max_sparsities = [float(s) for s in config.layer_max_sparsities.split(',')]

            if config.permanent_sparsification:
                for idx, layer_max_sparsity in enumerate(config.layer_max_sparsities[:-1]):
                    config.size_hidden[idx] = round((1 - layer_max_sparsity)*config.size_hidden[idx])
                    config.layer_max_sparsities[idx] = 0
        ...
        config.lr_rec = misc.str_to_floats(config.lr_rec)
...
def check_invalid_args_general(config, network_type):
    ...
    if network_type == 'DFC' or network_type == 'DFC_single_phase':
        ...
        if config.layer_max_sparsities and \
        (len(config.layer_max_sparsities) != len(config.size_hidden) + 1):
            raise ValueError('If `layer_max_sparsities` is provided, it must equal '
                            '`len(config.size_hidden) + 1`.')
