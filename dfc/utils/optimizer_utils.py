...
def get_optimizers(config, net, network_type='BP', logger=None):
    ...
    if 'DFC' in network_type:
        ...
        if net.use_recurrent_weights:
            recurrent_params = extract_parameters(net, config, network_type,
                                                params_type='recurrent',
                                                return_nones=True)
            
            recurrent_optimizer = OptimizerList(recurrent_params,
                                lr=config.lr_rec, 
                                optimizer_type=config.optimizer_rec,
                                no_bias=True,
                                momentum=config.momentum_rec,
                                weight_decay=config.weight_decay_rec,
                                adam_beta1=config.adam_beta1_rec,
                                adam_beta2=config.adam_beta2_rec,
                                adam_epsilon=config.adam_epsilon_rec)

            optimizers['recurrent'] = recurrent_optimizer
...
def extract_parameters(net, config, network_type, params_type='forward',
    ...
    if network_type == 'BP':
        ...
        if params_type == 'forward':
            ...
            params = net.recurrent_params
            params[-1] = []  # deactivate recurrent weight learning in output layer
            #params[0] = []  # deactivate recurrent weight learning in first hidden layer
...
class OptimizerList(object):
    ...
    def __init__(self, params_list, lr=1e-3, optimizer_type='SGD',
        ...
        for i, params in enumerate(params_list):
            if not (params is None or len(params) == 0):
