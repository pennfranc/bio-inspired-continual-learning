...
def update_summary_info_cl(config, shared, network_type):
    """Write information of incremental training into the summary
    dictionary.
    ...
    for i in range(config.num_tasks):
        summary[f'task_{i}_test_loss'] = getattr(tv, f'task_{i}_test_loss')
        summary[f'task_{i}_test_accu'] = getattr(tv, f'task_{i}_test_accu')
        summary[f'task_{i}_train_loss'] = getattr(tv, f'task_{i}_train_loss')
        summary[f'task_{i}_train_accu'] = getattr(tv, f'task_{i}_train_accu')

    summary['task_test_loss'] = tv.task_test_loss
