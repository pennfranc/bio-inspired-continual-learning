...
purposes. We mainly use it to run searches in clusters and save the results
in a CSV file.
...
.. code-block:: console
   ...
   $ bsub -n 1 -W 48:00 -e hpsearch.err -o hpsearch.out \\
     -R "rusage[mem=8000]" \\
     python3 hpsearch_hypnettorch.py --run_cluster --num_jobs=20 \\
     --grid_module=configs.icml_hpsearch.mnist_bp_euler --out_dir=out
...
def run(argv=None, dout_dir='./out/hyperparam_search'):
    ...
    for i, cond in enumerate(orig_conditions):
        ...
        # Conditions can be provided as tuples of dictionaries, which will
        # indicate exact values to be set, or as tuples of strings, in which
        # case the command will be removed if the value for the first element
        # or key is smaller or equal than the value of the second element.
        assert len(cond) == 2 and (isinstance(cond[0], dict) \
               and isinstance(cond[1], dict)) or (isinstance(cond[0], str) \
        ...
        if isinstance(cond, dict):
            ...
            for j, command in enumerate(commands):
                ...
                if cond in comm_keys and action in comm_keys:
                    ...
                    if command[cond] > command[action]:
    ...
    try:
        ...
        cols_reordered = ['finished', 'acc_test_val_best']
