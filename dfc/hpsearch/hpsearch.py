...
.. code-block:: console
   ...
   $ python3 -m hpsearch.hpsearch --num_jobs=20 \\
...
.. code-block:: console
   ...
   $ bsub -n 1 -W 48:00 -e hpsearch.err -o hpsearch.out \\
     -R "rusage[mem=8000]" \\
     python3 -m hpsearch.hpsearch --run_cluster --num_jobs=20 \\
     --grid_module=configs.hpsearch.hpconfig_template

For more demanding jobs one may request more resources:
...
And note that if the grid to be evaluated is too large one can add the option 
``--dont_generate_full_grid``.

Please fill in the grid parameters in the corresponding config file (see
command line argument `grid_module`).
"""
# torch must be imported first even if it does nothing in this file
import torch
...
def run(argv=None, dout_dir='./out/hyperparam_search'):
    ...
    for i, cond in enumerate(orig_conditions):
        ...
        # Conditions can be provided in three different ways:
        # 1) as 2-tuples of dictionaries, which indicate exact values to be set
        # 2) as 2-tuples of strings, in which case the command will be removed
        #    if the value for the first element or key is smaller or equal than
        #    the value of the second element.
        # 3) as 3-tuples with two strings and a value, performing a similar
        #    operation as in 2), but where the last value indicates the scaling
        #    factor, i.e. the first key-value pair has to be bigger than the
        #    second one by a factor indicated by this last value.
        assert len(cond) == 2 or \
                (len(cond) == 3 and isinstance(cond[2], float)) or \
                (len(cond) == 3 and isinstance(cond[2], int))
        assert (isinstance(cond[0], dict) and isinstance(cond[1], dict)) or \
               (isinstance(cond[0], str) and isinstance(cond[1], str))
        valid = True

        if isinstance(cond[0], dict):
            cond_keys = cond[0].keys()
        elif isinstance(cond[0], str):
            cond_keys = [cond[0]]
        for k in cond_keys:
            if k not in grid.keys():
                warnings.warn('Condition %d can not be enforced. ' % (i) +
                    'Key "%s" is not specified in grid -- %s.' % \
                    (k, str(cond)))
                valid = False
        if valid:
    ...
    for i, cond_tup in enumerate(conditions):
        if len(cond_tup) == 2:
            ...
            cond, action, factor = cond_tup

        if isinstance(cond, dict):
            ## Option 1: setting desired values
            ...
            new_commands = []
            ## Option 2: removing runs that don't fulfill greater than condition
            for j, command in enumerate(commands):
                comm_keys = command.keys()
                if cond in comm_keys and action in comm_keys:
                    # Enforce greater than condition.
                    if command[cond] > factor * command[action]:
                        new_commands.append(command)
                else:
                    # If condition keys don't exist in this command, add the
                    # command to the list.
                    new_commands.append(command)
    ...
    try:
        ...
        ### Write a new column order starting with main performance result.
        cols = csv_file_content.columns.tolist()
        cols_reordered = ['finished']
        if 'acc_test_val_best' in cols:
            cols_reordered.append('acc_test_val_best')
        for key in _SUMMARY_KEYWORDS:
            if key not in cols_reordered and key in cols:
                cols_reordered.append(key)
        # Add command line parameters.
        for key in cols:
            if key not in cols_reordered:
                cols_reordered.append(key)
        assert len(cols) == len(cols_reordered)
        csv_file_content = csv_file_content[cols_reordered]
