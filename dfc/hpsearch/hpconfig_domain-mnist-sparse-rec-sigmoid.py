...
grid = {
    ...
    'batch_size': [512],
    'lr': [1 / 10**(i / 2) for i in range(2, 10, 1)],
    'use_jacobian_as_fb': [True],
    'error_as_loss_grad': [True],
    'layer_max_sparsities': [f"{u/10},{v/10},0.5" for u in range(0, 10, 2) for v in range(0, 10, 2)],
    
    'use_recurrent_weights': [True],
...
conditions = [
    ...
    ({'use_recurrent_weights': [False]}, {'rec_adaptation': [-1],
     ...
     ({'permanent_sparsification': [True]}, {'layer_max_sparsities': [grid['layer_max_sparsities'][-1]],  # avoiding perm sparsification for no sparsity
